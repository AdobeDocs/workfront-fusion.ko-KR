...
>[!NOTE]
>
>시나리오 기록이 실행 중일 때 이 기록을 보면 데이터가 계속 처리 중임을 알리는 메모가 표시되고 처리가 완료될 때까지 부분 시나리오 기록만 표시됩니다.
...
1. (선택 사항) 날짜 범위별로 검색을 제한하려면 [!UICONTROL 날짜 범위별] 영역에서 원하는 검색의 시작 날짜와 종료 날짜를 선택합니다.
   ...
   >
   >* Workfront Fusion은 30일 동안 웹후크 페이로드를 저장합니다. 웹후크 페이로드가 생성된 후 30일 이상 지나면 &quot;[!UICONTROL 저장소에서 파일을 읽지 못했습니다.]&quot; 오류가 발생합니다.
