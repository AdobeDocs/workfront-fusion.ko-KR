...
* [사용자 정의 스크립트 실행 요청 제출](#submit-a-custom-script-execution-request)
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>렌디션에 추가할 각 에셋에 대해 다음 작업을 수행하십시오.<ol><li><b>항목 추가</b>를 클릭합니다.</li><li>에셋의 소스를 선택하거나 매핑합니다.</li><li>대상을 입력합니다. 대상은 리소스가 다운로드되는 임시 기본 디렉터리(작업 디렉터리)에 상대적인 경로입니다. 이는 매개 변수 내의 에셋을 식별합니다. '..' 또는 '/'를 사용하여 올라갈 수 없습니다. 유효한 파일 이름이 있어야 합니다.</td>
    </tr>
    <tr>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>데이터 병합에 추가할 각 자산에 대해 다음 작업을 수행하십시오.<ol><li><b>항목 추가</b>를 클릭합니다.</li><li>에셋의 소스를 선택하거나 매핑합니다.</li><li>대상을 입력합니다. 대상은 리소스가 다운로드되는 임시 기본 디렉터리(작업 디렉터리)에 상대적인 경로입니다. 이는 매개 변수 내의 에셋을 식별합니다. '..' 또는 '/'를 사용하여 올라갈 수 없습니다. 유효한 파일 이름이 있어야 합니다.</td>
    </tr>
    <tr>
      <td role="rowheader">대상 문서</td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>모듈에 추가할 각 에셋에 대해:<ol><li><b>항목 추가</b>를 클릭합니다.</li><li>에셋의 소스를 선택하거나 매핑합니다.</li><li>대상을 입력합니다. 대상은 리소스가 다운로드되는 임시 기본 디렉터리(작업 디렉터리)에 상대적인 경로입니다. 이는 매개 변수 내의 에셋을 식별합니다. '..' 또는 '/'를 사용하여 올라갈 수 없습니다. 유효한 파일 이름이 있어야 합니다.</td>
    </tr>
    <tr>
      <td role="rowheader">대상 문서</td>
...
<table>
  ...
  <tbody>
    ...
    <tr>
      ...
      <td>실행 요청을 제출하려는 각 에셋에 대해 <ol><li><b>항목 추가</b>를 클릭합니다.</li><li>에셋의 소스를 선택하거나 매핑합니다.</li><li>대상을 입력합니다. 대상은 리소스가 다운로드되는 임시 기본 디렉터리(작업 디렉터리)에 상대적인 경로입니다. 이는 매개 변수 내의 에셋을 식별합니다. '..' 또는 '/'를 사용하여 올라갈 수 없습니다. 유효한 파일 이름이 있어야 합니다.</td>
    </tr>
    <tr>
      <td role="rowheader">기타 필드</td>
   <td>다른 필드의 경우 모듈에 포함된 정보를 참조하십시오.</td>     </tr>
...
이 모듈은 PDF 문서를 편집 가능한 InDesign(INDD 또는 IDML) 형식으로 변환합니다. 출력은 변환된 문서 및 관련 에셋과 함께 각 입력 PDF의 이름을 딴 하위 폴더가 포함된 ZIP 파일(기본 이름 &quot;output.zip&quot;)입니다. 임베드 링크 옵션이 false인 경우 에셋이 ZIP 내의 개별 폴더에 제공됩니다. true이면 모든 링크가 InDesign 파일에 포함됩니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td role="rowheader">에셋 입력</td>
      <td>변환할 각 자산에 대해 <b>항목 추가</b>를 클릭하고 자산의 URL을 입력하고 로컬 파일 이름을 지정하십시오. 파일 이름은 모듈의 후반부에 참조됩니다.</td>
    </tr>
  <tr> 
   <td>대상 문서</td> 
   <td>변환할 각 문서에 대해 <b>항목 추가</b>를 클릭하고 입력 에셋 필드에 지정된 파일 이름을 입력합니다.</td> 
  </tr> 
  <tr> 
   <td>출력 형식</td> 
   <td>파일을 INDD 파일로 변환할지 IDML 파일로 변환할지 선택합니다.</td> 
  </tr> 
  <tr> 
   <td>링크 포함</td> 
   <td>모든 이미지 및 에셋 링크를 INDD 또는 IDML 파일에 직접 포함하려면 예를 선택합니다. 이러한 에셋을 ZIP 파일의 별도 폴더에 배치하려면 아니오를 선택합니다.</td> 
  </tr> 
  <tr> 
   <td>출력 ZIP 파일 이름</td> 
   <td>출력 ZIP 파일의 이름을 입력하거나 지정합니다.</td> 
  </tr> 
  <tr> 
   <td>출력</td> 
   <td>출력하려는 각 파일에 대해 저장소 유형을 선택하고 저장소 세부 정보를 입력합니다.</td> 
  </tr> 
  <tr> 
   <td>반환된 최대 결과 수</td> 
   <td>각 실행 주기에 대해 모듈이 반환할 최대 결과 수를 입력합니다.</td> 
  </tr> 
  </tbody>
</table>
...
이 모듈은 등록을 위해 사용자 지정 스크립트 번들을 제출하고 등록된 스크립트에 대한 실행 요청을 게시하기 위한 URL을 반환합니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td role="rowheader">스크립트 번들</td>
      <td>문서 다운로드 모듈과 같은 이전 모듈의 소스 파일을 매핑합니다. ZIP 파일이어야 합니다.</td>
    </tr>
  <tr> 
   <td>파일 이름</td> 
   <td>스크립트 번들이 포함된 업로드된 파일의 이름을 입력하거나 매핑합니다.</td> 
  </tr> 
  </tbody>
</table>
...
이 모듈은 등록된 사용자 지정 스크립트에 대한 InDesign 앱 버전 구성을 업데이트합니다. 이를 통해 최신 버전 사용, 주요 버전 수정 또는 특정 주요 및 부 버전 수정을 포함한 버전 전략을 지정할 수 있습니다.
...
<table>
  ...
  <tbody>
    ...
    <tr>
      <td role="rowheader">스크립트 이름</td>
      <td>업데이트할 사용자 지정 스크립트의 이름을 입력하거나 매핑합니다. 스크립트를 등록할 때 반환된 <code>capability</code> 값입니다.</td>
    </tr>
  <tr> 
   <td>앱 버전 전략</td> 
   <td>사용할 앱 버전 전략을 선택합니다.
   <ul>
   <li><b>항상 최신 버전 사용</b></li>
   <li><b>주요 버전에 고정</b><p>이를 적용할 주요 버전에 대한 번호를 입력하거나 매핑합니다.</p></li>
   <li><b>주 버전 및 부 버전에 고정</b><p>적용할 주 버전 및 부 버전을 입력합니다.</p></li>
   </ul></td> 
  </tr> 
  </tbody>
</table>
...
이 모듈은 등록된 각 앱 버전에 대한 주요 버전, 부 버전 및 상태를 포함하여 사용 가능한 모든 InDesign 앱 버전에 대한 정보를 검색합니다.
