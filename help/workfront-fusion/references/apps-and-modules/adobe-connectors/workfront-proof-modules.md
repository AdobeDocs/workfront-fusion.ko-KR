...
description: Adobe Workfront Fusion 시나리오에서는  [!DNL Workfront Proof]를 사용하는 워크플로를 자동화할 수 있으며 여러 제3자 애플리케이션 및 서비스에 연결할 수 있습니다.
author: Becky
...
Adobe Workfront Fusion 시나리오에서는 [!DNL Workfront Proof]를 사용하는 워크플로를 자동화할 수 있으며 여러 제3자 애플리케이션 및 서비스에 연결할 수 있습니다.

특정 이벤트를 기반으로 증명 업데이트 및 증명 수신자 검색 등 Workfront 또는 [!DNL Workfront Proof]에서 현재 증명에서 지원되지 않는 작업을 실행해야 하는 경우 유용합니다.
...
## [!DNL Workfront Proof]를 Workfront Fusion에 연결
...
[!DNL Workfront Proof] 모듈을 구성할 때 Workfront Fusion은 아래 나열된 필드를 표시합니다. 이와 함께 앱 또는 서비스의 액세스 레벨과 같은 요인에 따라 추가적인 [!DNL Workfront Proof] 필드가 표시될 수 있습니다. 모듈의 굵은 글씨 제목은 필수 필드를 나타냅니다.
...
모듈은 증명과 연결된 모든 표준 필드와 연결이 액세스하는 모든 사용자 지정 필드 및 값을 반환합니다. 또한 PDF 요약에 대한 새 이벤트 구독을 만들고 페이로드로 전송된 `pdf_url` 특성에서 콘텐츠를 출력합니다. 시나리오의 후속 모듈에서 이 정보를 매핑할 수 있습니다.
...
<table style="table-layout:auto">
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p>[!DNL Workfront Proof] 계정을 Workfront Fusion에 연결하는 방법에 대한 지침은 <a href="/help/workfront-fusion/create-scenarios/connect-to-apps/connect-to-fusion-general.md" class="MCXref xref" data-mc-variable-override="">Adobe Workfront Fusion에 연결 만들기 - 기본 지침</a>을 참조하십시오.</p> </td> 
  </tr> 
 </tbody> 
</table>
...
모듈은 지정한 기간 동안 찾은 모든 레코드 목록과 함께 해당 유형을 반환합니다. 또한 지정한 필드의 값도 반환합니다. 증명에서 결정된 사항이 모듈에서 발견된 경우, 별도의 필드에 이전 값과 현재 값을 모두 포함합니다. 시나리오의 후속 모듈에서 이 정보를 매핑할 수 있습니다.
...
이 액션 모듈을 사용하면 [!DNL Workfront Proof] API에 인증된 사용자 정의 호출을 수행할 수 있습니다. 이렇게 하면 다른 [!DNL Workfront Proof] 모듈로는 수행할 수 없는 데이터 흐름 자동화를 만들 수 있습니다.

모듈은 상태 코드, 헤더 및 본문을 반환합니다. 시나리오의 후속 모듈에서 이 정보를 매핑할 수 있습니다.
...
이 모듈은 증명에 대해 선택한 필드의 값과 해당 유형을 반환합니다. 시나리오의 후속 모듈에서 이 정보를 매핑할 수 있습니다.
...
모듈이 PDF 요약 정보를 반환합니다. 시나리오의 후속 모듈에서 이 정보를 매핑할 수 있습니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  <tr> 
   <td>[!DNL Connection]</td> 
   <td> <p>[!DNL Workfront Proof] 계정을 Workfront Fusion에 연결하는 방법에 대한 지침은 <a href="/help/workfront-fusion/create-scenarios/connect-to-apps/connect-to-fusion-general.md" class="MCXref xref" data-mc-variable-override="">Adobe Workfront Fusion 연결 만들기 - 기본 지침</a>을 참조하십시오.</p> </td> 
  </tr> 
  <tr> 
   <td>[!UICONTROL 출력]</td> 
   <td> <p>이 모듈의 출력 번들에 포함할 정보를 선택합니다.</p> </td> 
