...
* [!DNL Adobe]을(를) 처음 사용하는 경우 조직의 [!DNL Adobe] 소프트웨어 및 서비스를 관리할 수 있는 관리 권한이 있다는 전자 메일을 받은 경우 전자 메일의 버튼을 클릭하여 [!DNL Adobe] 계정을 만들고 [!DNL Admin Console]을(를) 여십시오.
...
1. (선택 사항) 계속해서 [Workfront Fusion에서 사용자 액세스 수준을 변경](#change-a-users-access-level-in-workfront-fusion)합니다.
