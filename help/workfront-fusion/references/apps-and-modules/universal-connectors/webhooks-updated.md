...
웹후크를 사용하여 앱을 Workfront Fusion에 연결하려면 클라이언트 인증서(mTLS), 기본 인증, Adobe Identity Management 시스템(IMS), API 키 또는 HMAC 서명을 사용하여 인증하도록 웹후크를 설정할 수 있습니다.

>[!NOTE]
>
>**API 키 인증이 이제 새 웹후크의 기본 인증 유형**&#x200B;입니다. 이전에는 권한 부여가 미리 선택되지 않았습니다. Webhook의 인증 유형을 다른 유형으로 변경하거나 인증 없음을 위해 빈 값을 선택할 수 있습니다.

* [Webhook 구성](#configure-a-webhook)
...
* [API 키 인증과 함께 웹후크 사용](#use-a-webhook-with-api-key-authentication)
* [HMAC 서명 인증과 함께 웹후크 사용](#use-a-webhook-with-hmac-signature-authentication)
...
1. (선택 사항) **[!UICONTROL 허용된 원본]** 필드에서 이 웹후크 호출을 허용할 각 원본에 대해 **항목 추가**&#x200B;를 클릭하고 원본 패턴을 입력하십시오. 원본을 허용하려면 이 필드를 비워 둡니다.
...
API 키 인증은 요청 헤더 또는 쿼리 매개 변수로 전송된 단일 키로 웹후크 끝점을 보호합니다. 새 웹후크의 기본 인증 유형입니다.
...
1. 들어오는 데이터의 유효성을 검사하려면 **데이터 구조** 필드에서 사용할 데이터 구조를 선택하거나 추가하십시오.
   ...
   데이터 구조에 대한 자세한 내용은 [데이터 구조](/help/workfront-fusion/references/mapping-panel/data-types/data-structures.md)를 참조하십시오.
1. 아직 선택하지 않은 경우 **인증 유형** 필드에서 **[!UICONTROL API 키 인증]**&#x200B;을 선택합니다.
1. **키** 필드에서 인증에 사용할 API 키를 선택하거나 **추가**&#x200B;를 클릭하고 새 자격 증명을 입력하여 새 API 키를 추가합니다.
   1. 새 자격 증명 키의 이름을 입력합니다.
   1. **키** 필드에 인증할 키 값을 입력하십시오. 필드 옆에 있는 눈 아이콘을 사용하여 입력하는 동안 표시하거나 숨깁니다.
   1. **API 키 배치** 필드에서 키를 헤더로 전송할지 또는 쿼리 매개 변수로 전송할지를 선택합니다.
   1. **API 키 매개 변수 이름** 필드에 키를 보낼 헤더 또는 쿼리 매개 변수 이름을 입력합니다(예: `X-API-Key`).
   1. **키 만들기**&#x200B;를 클릭합니다.
   1. 웹후크 패널의 **자격 증명** 필드에서 새 키를 선택합니다.
1. 원하는 대로 다른 설정을 활성화합니다.
1. **[!UICONTROL 저장]**&#x200B;을 클릭합니다
...
#### HMAC 서명 인증과 함께 웹후크 사용

HMAC 서명 인증은 들어오는 요청이 공유 서명 암호로 서명되었는지 확인하므로, 모든 요청에서 암호 자체를 전송하지 않고 변조 및 스푸핑된 호출을 방지할 수 있습니다.
...
1. 들어오는 데이터의 유효성을 검사하려면 **데이터 구조** 필드에서 사용할 데이터 구조를 선택하거나 추가하십시오.
   ...
   데이터 구조에 대한 자세한 내용은 [데이터 구조](/help/workfront-fusion/references/mapping-panel/data-types/data-structures.md)를 참조하십시오.
1. **인증 유형** 필드에서 **[!UICONTROL HMAC 서명]**&#x200B;을(를) 선택합니다.
1. **key** 필드에서 인증에 사용할 서명을 선택하거나 **추가**&#x200B;를 클릭하고 새 자격 증명을 입력하여 새 서명을 추가하십시오.
   1. 새 자격 증명 키의 이름을 입력합니다.
   1. **서명 암호** 필드에 사용할 공유 암호를 입력하십시오. 필드 옆에 있는 눈 아이콘을 사용하여 입력하는 동안 표시하거나 숨깁니다.
   1. **알고리즘** 필드에서 사용할 해싱 알고리즘(예: SHA-256)을 선택합니다.
   1. **서명 헤더** 필드에 서명을 읽을 헤더의 이름을 입력합니다(예: `x-fusion-signature-256`).
   1. **서명 인코딩** 필드에서 서명 값의 인코딩을 선택합니다(예: 16진수).
   1. (선택 사항) **서명 접두사** 필드에 `sha256=`과 같이 서명 값 접두사가 필요한 경우 접두사를 입력합니다.
   1. **키 만들기**&#x200B;를 클릭합니다.
   1. 웹후크 패널의 **자격 증명** 필드에서 새 키를 선택합니다.
1. 원하는 대로 다른 설정을 활성화합니다.
1. **[!UICONTROL 저장]**&#x200B;을 클릭합니다
