...
### 조직 소유자 추가 또는 변경 시 고려 사항

조직에 두 명 이상의 소유자가 있을 수 있습니다.

소유자 역할에 사용자를 할당하거나 소유자 역할에서 사용자를 할당할 때 다음 사항을 고려하십시오.

* 소유자만 소유자 역할을 할당하거나 현재 소유자에게 다른 역할을 할당할 수 있습니다.
* 관리자만 소유자로 업그레이드할 수 있습니다.
* 소유자가 한 명만 있는 경우 해당 소유자 역할을 변경하거나 제거할 수 없습니다.
* 소유자가 한 명만 있는 경우 조직에 다른 사용자가 있는 경우 해당 소유자를 삭제할 수 없습니다.
* 관리자에게 소유자 역할이 할당되면 조직 내의 모든 팀에서 자동으로 관리자가 됩니다.
* 소유자가 다른 역할에 할당되면 해당 사용자는 자동으로 모든 팀의 팀원이 됩니다.
* 새 팀이 생성되면 모든 소유자는 자동으로 팀 관리자로 할당됩니다.
