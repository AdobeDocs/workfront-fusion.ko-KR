...
* 시나리오 체인 기능은 Beta에 있으며 미션 크리티컬 워크플로우에는 권장되지 않습니다. Beta 기능에서는 동작이 변경되고 극단적 사례는 완전히 처리되지 않을 수 있습니다.

  안정적인 통합을 위해 HTTP 요청 모듈을 사용하여 Webhook을 통해 두 번째 시나리오를 트리거하는 것이 좋습니다. 이 패턴은 완전히 지원되는 프리미티브를 사용하고 각 시나리오에 독립적인 실행 제어를 제공합니다.

  체인 시나리오를 사용하려면 문서 [여러 시나리오를 함께 연결](/help/workfront-fusion/create-scenarios/plan-a-scenario/chain-scenarios.md)에서 디자인 지침과 제약 조건을 검토하십시오.
...
* 웹후크의 큐에 대기 중인 이벤트를 최대 **100,000**&#x200B;개까지 저장할 수 있습니다. Webhook을 사용하는 시나리오가 꺼져 있거나, 즉시가 아닌 일정에 따라 실행되도록 설정된 경우 큐가 채워집니다. 큐가 100,000개의 이벤트에 도달하면 &quot;큐가 가득 찼습니다&quot; 오류와 400개의 상태 코드로 새 이벤트가 거부됩니다.

  Workfront 및 Planning 이벤트의 경우 큐가 장기간 꽉 차면 이벤트 구독이 비활성화되고, 동결되며, Workfront Fusion에서 해당 구독에 대한 이벤트 수신을 중단합니다.
...
* 시나리오가 연결된 네트워크의 일부인 경우 실행 기록은 체인의 각 시나리오별로 개별적으로 유지됩니다. 상위 및 하위 시나리오 간에 통합된 추적 보기가 없습니다. 연결된 실행을 조사하려면 각 시나리오의 실행 기록을 개별적으로 엽니다.
* 단일 작업의 입력 또는 출력이 15MB를 초과하면 실행 기록에 나타나지 않습니다. 이 제한은 체인 모듈을 통해 상위 및 하위 시나리오 사이에 전달되는 데이터에 적용됩니다.
