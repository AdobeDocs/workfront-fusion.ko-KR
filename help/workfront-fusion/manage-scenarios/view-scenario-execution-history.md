...
[!UICONTROL History] 탭에는 [!UICONTROL Scenario detail] 페이지에서 사용할 수 있는 것보다 더 많은 세부 정보가 표시됩니다. [!UICONTROL History] 탭에서 실행을 필터링하고 정렬할 수도 있습니다.

1. 왼쪽 패널의 **[!UICONTROL Scenario]** 탭을 클릭한 다음 시나리오를 클릭합니다.
...
1. 시나리오 이름 근처에 있는 **기록**을 클릭합니다.
   ...
   * 실행이 **[!UICONTROL Started]**&#x200B;인 날짜
   * 실행 ID
   * **[!UICONTROL Status]**(성공 또는 실패)
   * **[!UICONTROL Duration]** 실행
   * **[!UICONTROL Operations]**&#x200B;의 수
   * **[!UICONTROL Data Transfer]**&#x200B;의 크기
...
1. 특정 시나리오 실행에 대한 세부 정보를 보려면 맨 오른쪽에 있는 **세부 정보**&#x200B;를 클릭하십시오. [!UICONTROL details] 링크는 실행에 사용 가능한 세부 정보가 있는 경우에만 표시됩니다.
...
1. 이 문서의 [!UICONTROL History] 탭에서 [시나리오 실행 기록 보기](#view-scenario-history-on-the-history-tab)에 설명된 대로 시나리오에 대한 전체 페이지 기록을 엽니다.
1. 실행 목록의 맨 위에 있는 **[!UICONTROL Fulltext search]**&#x200B;을(를) 클릭합니다.

   또는

   **Ctrl+Shift+F**(Windows) 또는 **Cmd+Shift+F**(Mac) 입력
[!UICONTROL Search in history] 창이 열립니다.

1. (선택 사항) 특정 텍스트가 포함된 실행을 검색하려면 **[!UICONTROL Search in history]** 창의 검색 막대에 텍스트를 입력하십시오.

   정확한 텍스트를 검색하려면 텍스트를 큰따옴표(&quot;예&quot;)로 둘러쌉니다.

   >[!INFO]
   >
   >**예:** 특정 프로젝트를 만든 실행을 찾으려면 [!UICONTROL Fulltext search] 막대에 프로젝트 ID를 입력하십시오.
   >
   >&quot;625ef2ef0006036bd1794b6e52d737c5&quot;

1. (선택 사항) 날짜 범위로 검색을 제한하려면 [!UICONTROL By date range] 영역에서 원하는 검색의 시작 날짜와 종료 날짜를 선택합니다.

   >[!NOTE]
   >
   >* 이전 30일 동안만 실행할 수 있습니다.
   >
   >* [!DNL Workfront Fusion]이(가) 30일 동안 webhook 페이로드를 저장합니다. 웹후크 페이로드가 생성된 후 30일 이상 지나면 &quot;[!UICONTROL Failed to read file from storage.]&quot; 오류가 발생합니다.


1. (선택 사항) 상태별로 검색을 제한하려면 **[!UICONTROL By status]** 드롭다운에서 원하는 상태를 선택합니다.
...
1. (선택 사항) **[!UICONTROL Sort by dates]** 드롭다운에 표시되는 순서를 변경합니다.

1. (선택 사항) 시나리오 실행 ID를 복사하려면 **[!UICONTROL Copy execution ID]** 아이콘을 클릭합니다 원하는 실행 행에 <img src="assets/copy-fusion-execution-id-icon.png">이(가) 있습니다.

1. (선택 사항) [!UICONTROL Fulltext search] 결과를 클릭하여 정보가 포함된 시나리오 모듈 출력 번들을 검사합니다.
