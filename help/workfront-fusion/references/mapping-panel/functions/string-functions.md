...
### [!UICONTROL 오른쪽(문자열; 길이)]

[!BADGE 새로 만들기!]{type=Informative}

문자열의 오른쪽에서 지정된 문자 수를 반환합니다.
...
제공된 문자열을 오름차순(알파벳)으로 정렬하여 반환합니다.
...
제공된 문자열을 내림차순(알파벳 역순)으로 정렬하여 반환합니다.

**구문:** `sortDescString(string1; string2; ...)`
...
각 단어의 첫 번째 문자를 대문자로 사용하고 공백을 제거하여 문자열을 PascalCase로 변환합니다.
