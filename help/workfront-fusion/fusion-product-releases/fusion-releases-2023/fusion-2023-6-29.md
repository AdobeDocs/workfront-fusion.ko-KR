...
# Workfront Fusion 릴리스 활동: 2023년 6월 26일 주간
...
* PDF에서 이미지 모듈로 업데이트됨: 이제 PDF에서 이미지 모듈로 이동하여 zip 파일을 출력할지 이미지 목록을 출력할지 여부를 선택할 수 있습니다.
* 새로운 PDF 분할 모듈: 이 모듈을 사용하여 PDF을 더 작은 PDF로 분할할 수 있습니다. 파일 수, 파일당 페이지 수 또는 페이지 범위별로 분할할 수 있습니다.
* 새로운 PDF 접근성 자동 태그 모듈: 이 모듈을 사용하여 PDF에 태그를 지정하고 접근성에 대한 보고서를 만들 수 있습니다. 접근성을 위해 태그가 지정된 PDF는 머리글이나 표와 같은 레이아웃 기능을 식별할 때 화면 판독기의 정확성을 향상시킵니다.
