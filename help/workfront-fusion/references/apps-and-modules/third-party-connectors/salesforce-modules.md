...
이 트리거 모듈은 오브젝트 내의 레코드가 생성되거나 업데이트될 때 시나리오를 실행한다. 모듈은 연결에서 액세스하는 모든 사용자 정의 필드 및 값과 함께 레코드와 연결된 모든 표준 필드를 반환합니다. 시나리오의 후속 모듈에서 이 정보를 매핑할 수 있습니다.
...
이 트리거 모듈은 누군가가 메시지를 보낼 때 시나리오를 실행합니다. 모듈은 연결에서 액세스하는 모든 사용자 정의 필드 및 값과 함께 레코드와 연결된 모든 표준 필드를 반환합니다. 시나리오의 후속 모듈에서 이 정보를 매핑할 수 있습니다.
...
모듈을 사용하면 모듈에서 사용할 수 있는 오브젝트의 필드를 선택할 수 있습니다. 이렇게 하면 모듈을 설정할 때 스크롤해야 하는 필드의 수가 줄어듭니다.
...
이 액션 모듈을 사용하면 [!DNL Salesforce] API에 인증된 사용자 정의 호출을 수행할 수 있습니다. 이렇게 하면 다른 [!DNL Salesforce] 모듈로는 수행할 수 없는 데이터 흐름 자동화를 만들 수 있습니다.
...
* **[!UICONTROL 본문]**(오브젝트): 모듈을 구성할 때 선택한 HTTP 요청에 따라 일부 데이터를 반환받을 수 있습니다. [!UICONTROL GET] 요청의 데이터와 같은 데이터가 이 개체에 포함되어 있습니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   ...
   <td> <p><code> &lt;Instance URL&gt;/services/data/v62.0/</code>와 관련된 경로를 입력합니다.</p> <p>사용 가능한 끝점 목록은 <a href="https://developer.salesforce.com/docs/atlas.en-us.api_rest.meta/api_rest/intro_what_is_rest_api.htm">Salesforce REST API 개발자 안내서</a>를 참조하십시오.</p> </td> 
  </tr> 
  <tr> 
  ...
  <tr> 
   ...
   <td> <p>표준 JSON 오브젝트 형식으로 API 호출에 대한 쿼리를 추가합니다. 예: <code>{"name":"something-urgent"}</code></p> </td> 
  </tr> 
  <tr> 
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td>[!UICONTROL ID]</td> 
   <td> <p>모듈에서 삭제할 레코드의 고유 [!DNL Salesforce] ID를 입력하거나 매핑합니다.</p> <p>ID를 가져오려면 브라우저에서 [!DNL Salesforce] 개체를 열고 마지막 슬래시(/) 뒤의 URL 끝에 있는 텍스트를 복사합니다. 예: <code>https://eu5.salesforce.com/&lt;object ID&gt;</code></p> </td> 
  </tr> 
 </tbody> 
</table>
...
모듈은 연결이 액세스하는 사용자 지정 필드 및 값과 함께 첨부 파일 또는 문서의 ID와 관련 필드를 반환합니다. 시나리오의 후속 모듈에서 이 정보를 매핑할 수 있습니다.
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr>
    ...
    <td> <p>모듈을 다운로드할 레코드의 고유 [!DNL Salesforce] ID를 입력하거나 매핑합니다.</p> <p>ID를 가져오려면 브라우저에서 [!DNL Salesforce] 개체를 열고 마지막 슬래시(/) 뒤의 URL 끝에 있는 텍스트를 복사합니다. 예: <code>https://eu5.salesforce.com/&lt;object ID&gt;</code></p> </td>
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr>
    <td>[!UICONTROL ID]</td>
    <td> <p>모듈에서 읽을 레코드의 고유 [!DNL Salesforce] ID를 입력하거나 매핑합니다.</p> <p>ID를 가져오려면 브라우저에서 [!DNL Salesforce] 개체를 열고 마지막 슬래시(/) 뒤의 URL 끝에 있는 텍스트를 복사합니다. 예: <code>https://eu5.salesforce.com/&lt;object ID&gt;</code></p> </td>
  </tr> 
 </tbody> 
</table>
...
<table style="table-layout:auto"> 
 ...
 <tbody> 
  ...
  <tr> 
   <td role="rowheader">[!UICONTROL 최대]</td> 
...
이 검색 모듈은 지정한 검색 쿼리와 일치하는 [!DNL Salesforce]의 개체에서 레코드를 찾습니다. 시나리오의 후속 모듈에서 이 정보를 매핑할 수 있습니다.
